...
Go ahead and resize the stage and notice how the rectangle is always half as wide and half as tall as the stage. In this example, we defined variables `width` and `height` even though we only used them once. However, defining them makes the last line `$rect(width, height)` much easier to read! Otherwise, we'd have to write:

```js
$rect($stageWidth / 2, $stageHeight /2);
```
